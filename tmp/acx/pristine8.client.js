...
// Text and surfaces follow the shipped token vocabulary. Every var() carries a
// fallback so a token this deployment does not define can never collapse a
// foreground onto its own background.
const CSS = [
...
return {
  apply(ctx) {
    ...
    // Every Host call carries a deadline: a Host half that never answers becomes a
    // reported error instead of an interface stuck on "creating".
    ...
    // Workspace services can await their own readiness snapshots; bound the whole
    // path so a stalled promise degrades into a reported error, never a hang.
    ...
    async function refresh() {
      ...
      for (let i = 0; i < roots.length; i++) {
        const root = roots[i]
        const r = await hostCall('canvas-list-files', { path: root, maxDepth: 2, limit: 160 }, 15000)
        if (!r.ok) {
    ...
    function CanvasApp() {
      ...
      React.useEffect(function () {
        ...
        if (form.kind === 'project') {
          hostCall('canvas-sessions', {}, 15000).then(function (r) {
            if (!alive) return
      ...
      async function createAgent(mission, cwdInput, preset) {
        ...
        const attempt = async function (cwd) {
          const r = await hostCall('canvas-create-session', { cwd: cwd, agentPreset: preset || null }, 15000)
          if (r.ok && r.sessionId) return r
          errors.push((cwd ? 'cwd=' + cwd : 'default cwd') + ': ' + (r.error || 'no session id'))
          if (r.timeout) {
            const relist = await hostCall('canvas-sessions', {}, 8000)
      ...
      async function submitAgentForm() {
        ...
        let r = null
        try {
          r = await createAgent(mission, formCwd, formPreset)
        } catch (e) {
    ...
    console.log('[agent-canvas] client apply pkg-7: panel "' + PANEL_ID + '" registered, touch=' + String(detectTouch()))
  },
}
