...
const CSS = [
  '.acx-root{position:relative;display:flex;height:100%;min-height:0;width:100%;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);font-size:13px;line-height:1.45;overflow:hidden}',
  '.acx-col{display:flex;flex-direction:column;flex:1;min-width:0;min-height:0;position:relative}',
  '.acx-bar{display:flex;align-items:center;gap:8px;padding:7px 10px;border-bottom:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);flex-wrap:wrap;flex:none}',
  '.acx-bar b{font-size:13px;font-weight:600;margin-right:2px}',
  '.acx-btn{display:inline-flex;align-items:center;gap:5px;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary);border-radius:6px;padding:3px 9px;font:inherit;font-size:12px;cursor:pointer;white-space:nowrap}',
  '.acx-btn:hover:not([disabled]){border-color:var(--dsw-alias-brand-primary);color:var(--dsw-alias-brand-primary)}',
  '.acx-btn[disabled]{opacity:.5;cursor:default}',
  '.acx-primary{background:var(--dsw-alias-brand-primary);border-color:var(--dsw-alias-brand-primary);color:#fff}',
  '.acx-primary:hover:not([disabled]){color:#fff;opacity:.9}',
  '.acx-danger{color:var(--dsw-alias-state-error-primary);border-color:var(--dsw-alias-state-error-primary)}',
  '.acx-spacer{flex:1}',
  '.acx-form{flex:none;max-height:58%;overflow:auto;border-bottom:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);padding:10px;display:flex;flex-direction:column;gap:8px}',
  '.acx-form-title{font-weight:600;font-size:13px}',
  '.acx-canvas{position:relative;flex:1;min-height:0;overflow:hidden;cursor:grab;background-color:var(--dsw-alias-bg-base);background-image:radial-gradient(var(--dsw-alias-border-l1) 1px,transparent 1px);background-size:20px 20px}',
  ...
  '.acx-file{position:absolute;box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);border-radius:8px;padding:5px 7px;display:flex;flex-direction:column;gap:2px;overflow:hidden;cursor:grab;user-select:none;touch-action:none}',
  '.acx-file[data-selected=1]{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}',
  '.acx-file[data-new=1]{border-color:var(--dsw-alias-state-success-primary)}',
  '.acx-file[data-dragging=1]{cursor:grabbing;z-index:4;box-shadow:0 8px 20px rgba(0,0,0,.2)}',
  ...
  '.acx-fmeta{font-size:10px;color:var(--dsw-alias-label-secondary);display:flex;align-items:center;gap:5px}',
  '.acx-agent{position:absolute;width:56px;height:56px;border-radius:50%;display:flex;align-items:center;justify-content:center;border:2px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);cursor:grab;user-select:none;box-sizing:border-box;touch-action:none}',
  '.acx-agent>span{font-size:15px;font-weight:600}',
  '.acx-agent[data-selected=1]{box-shadow:0 0 0 3px rgba(99,102,241,.32)}',
  '.acx-agent[data-dragging=1]{cursor:grabbing;z-index:5;opacity:.92}',
  '.acx-agent-label{position:absolute;top:58px;left:-18px;width:92px;text-align:center;font-size:10.5px;color:var(--dsw-alias-label-secondary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
  '.acx-dot{position:absolute;right:0;top:0;width:11px;height:11px;border-radius:50%;border:2px solid var(--dsw-alias-bg-base)}',
  '.acx-badge{position:absolute;left:-4px;top:-4px;min-width:16px;height:16px;border-radius:8px;background:var(--dsw-alias-state-success-primary);color:#fff;font-size:10px;display:flex;align-items:center;justify-content:center;padding:0 4px}',
  '.acx-hover{position:absolute;width:320px;box-sizing:border-box;background:var(--dsw-alias-bg-overlay);border:1px solid var(--dsw-alias-border-l2);border-radius:10px;padding:9px;font-size:11.5px;line-height:1.5;z-index:20;box-shadow:0 8px 24px rgba(0,0,0,.22);pointer-events:none}',
  '.acx-hsect{margin-top:6px;padding-top:6px;border-top:1px dashed var(--dsw-alias-border-l1)}',
  '.acx-hlab{color:var(--dsw-alias-label-secondary);font-size:10px}',
  '.acx-mono{font-family:ui-monospace,Menlo,monospace;font-size:10px;word-break:break-all}',
  ...
  '.acx-dock{width:376px;flex:none;border-left:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);display:flex;flex-direction:column;min-height:0}',
  '.acx-dock-head{display:flex;align-items:center;gap:6px;padding:8px 10px;border-bottom:1px solid var(--dsw-alias-border-l1);font-weight:600;font-size:12.5px;flex:none}',
  '.acx-chips{display:flex;flex-wrap:wrap;align-items:center;gap:5px;padding:7px 10px;border-bottom:1px solid var(--dsw-alias-border-l1);flex:none}',
  '.acx-chip{display:inline-flex;align-items:center;gap:4px;border:1px solid var(--dsw-alias-border-l2);border-radius:999px;padding:1px 8px;font-size:11px;background:var(--dsw-alias-bg-layer-2)}',
  '.acx-log{flex:1;min-height:0;overflow:auto;padding:10px;display:flex;flex-direction:column;gap:8px}',
  '.acx-msg{border-radius:9px;padding:6px 9px;font-size:12px;white-space:pre-wrap;word-break:break-word;border:1px solid var(--dsw-alias-border-l1)}',
  '.acx-msg[data-role=user]{background:var(--dsw-alias-bg-layer-2);border-color:var(--dsw-alias-border-l2);align-self:flex-end;max-width:92%}',
  '.acx-msg[data-role=assistant]{background:transparent;max-width:96%}',
  '.acx-msg[data-role=tool]{font-size:11px;color:var(--dsw-alias-label-secondary);border-style:dashed;max-width:96%}',
  '.acx-msg[data-live=1]{border-color:var(--dsw-alias-brand-primary)}',
  '.acx-compose{border-top:1px solid var(--dsw-alias-border-l1);padding:9px;display:flex;flex-direction:column;gap:7px;flex:none}',
  ...
  '.acx-ok{color:var(--dsw-alias-state-success-primary)}',
  '.acx-err{color:var(--dsw-alias-state-error-primary)}',
  '.acx-warn{color:var(--dsw-alias-state-warn-primary)}',
  '.acx-hint{font-size:11.5px;color:var(--dsw-alias-label-secondary);line-height:1.5}',
  '.acx-ta{width:100%;min-height:86px;box-sizing:border-box;resize:vertical;background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:8px;font:inherit;font-size:12.5px}',
  '.acx-inp{background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:6px;padding:4px 7px;font:inherit;font-size:12px;box-sizing:border-box;flex:1}',
  '.acx-sel{background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:6px;padding:4px 6px;font:inherit;font-size:12px;box-sizing:border-box}',
  '.acx-btns{display:flex;align-items:center;gap:8px;flex-wrap:wrap}',
  '.acx-slist{display:flex;flex-direction:column;gap:5px;max-height:250px;overflow:auto;border:1px solid var(--dsw-alias-border-l1);border-radius:8px;padding:6px}',
  '.acx-srow{display:flex;flex-direction:column;gap:2px;padding:6px 9px;border:1px solid var(--dsw-alias-border-l1);border-radius:8px;cursor:pointer;background:var(--dsw-alias-bg-layer-1)}',
  '.acx-srow:hover{border-color:var(--dsw-alias-brand-primary)}',
  '.acx-st{font-size:12.5px}',
  '.acx-sm{font-size:10.5px;color:var(--dsw-alias-label-secondary);word-break:break-all}',
  '.acx-empty{color:var(--dsw-alias-label-secondary);font-size:12px;padding:12px;text-align:center}',
  '.acx-inline{border:1px solid var(--dsw-alias-border-l1);border-radius:10px;padding:10px;display:flex;flex-direction:column;gap:7px;font-size:12.5px;background:var(--dsw-alias-bg-layer-1)}',
  '.acx-toast{position:absolute;left:50%;bottom:18px;transform:translateX(-50%);background:var(--dsw-alias-bg-overlay);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:6px 12px;font-size:12px;z-index:40;max-width:80%;box-shadow:0 4px 14px rgba(0,0,0,.18)}',
  '.acx-errbox{border:1px solid var(--dsw-alias-state-error-primary);border-radius:8px;padding:7px 9px;font-size:11.5px;color:var(--dsw-alias-state-error-primary);white-space:pre-wrap;word-break:break-word}',
].join('\n')
...
return {
  apply(ctx) {
    ...
    // Baselines are mutable closure data: a path absent from its root baseline
    // is a file the agent created after the canvas started watching.
    const baselines = {}
    const manual = {}
    ...
    // Scan every agent working directory and rebuild the file-node projection.
    ...
    function CanvasIcon(props) {
      ...
      const color = props && props.active ? 'var(--dsw-alias-brand-primary)' : 'currentColor'
    ...
    function ChatDock() {
      ...
      const foot = h('div', { className: 'acx-compose' },
        ...
        h('div', { className: 'acx-row' },
          ...
          h('span', { className: 'acx-sm' }, busy ? '发送中...' : 'Enter 发送'),
          h('span', { className: 'acx-spacer' }),
          h('button', {
    ...
    function CanvasApp() {
      ...
      function onAgentPointerDown(e, agent) {
        ...
        e.stopPropagation()
        const p = point(e)
        dragRef.current = { id: agent.id, dx: p.x - agent.x, dy: p.y - agent.y }
      ...
      function onFilePointerDown(e, f) {
        ...
        fileDragRef.current = { path: f.path, dx: p.x - pos.x, dy: p.y - pos.y, moved: false }
      ...
      async function createAgent(mission, cwdInput, preset) {
        ...
        const attempt = async function (cwd) {
          const r = await hostCall('canvas-create-session', { cwd: cwd, agentPreset: preset || null })
        ...
        if (!res && target) {
          const r3 = await createViaWorkspace(target)
          if (r3.ok && r3.sessionId) res = { ok: true, sessionId: r3.sessionId }
          else errors.push('workspace: ' + (r3.error || 'no session id'))
        }
        if (!res) { used = null; res = await attempt(null) }
      ...
      async function submitAgentForm() {
        ...
        setFormBusy(true)
        setFormErr(null)
        const r = await createAgent(mission, formCwd, formPreset)
        setFormBusy(false)
        if (!r.ok) { setFormErr(r.error || 'create failed'); return }
        setForm(null)
        setFormMission('')
        store.set({ fileSel: [] })
      ...
      function renderAgent(a) {
        ...
        return h('div', {
          ...
          h('div', { className: 'acx-dot', style: { background: live ? 'var(--dsw-alias-state-success-primary)' : 'var(--dsw-alias-border-l2)' } }),
      ...
      function hoverAgentCard(a) {
        ...
        return h('div', { className: 'acx-hover', style: { left: (a.x + 84) + 'px', top: (a.y - 10) + 'px' } },
          ...
          h('div', { className: 'acx-sm' }, a.mission ? ('使命: ' + firstLine(a.mission, 80)) : '使命: (未设置)'),
          h('div', { className: 'acx-hsect' },
            ...
            mine.length > 0 ? h('div', { className: 'acx-flist' }, shown.map(function (f) {
              return h('div', { key: f.path, className: 'acx-frow' },
                ...
                h('span', { style: { color: 'var(--dsw-alias-label-secondary)' } }, f.rel || f.name),
              )
      ...
      function hoverFileCard(f) {
        ...
        return h('div', { className: 'acx-hover', style: { left: (pos.x + 20) + 'px', top: (pos.y + FILE_H + 6) + 'px' } },
          ...
          h('div', { className: 'acx-mono' }, f.path),
          h('div', { className: 'acx-sm' }, (f.type === 'directory' ? '目录' : '文件') + (f.size !== null && f.type !== 'directory' ? ' · ' + fmtSize(f.size) : '')),
          h('div', { className: 'acx-hsect' },
      ...
      const svg = h('svg', {
        ...
        return h('path', {
          ...
          fill: 'none',
          stroke: e.isNew ? 'var(--dsw-alias-state-success-primary)' : 'var(--dsw-alias-border-l2)',
      ...
      const bar = h('div', { className: 'acx-bar' },
        ...
        h('span', { className: 'acx-spacer' }),
        h('span', { className: 'acx-sm' }, 'agent ' + String(st.agents.length) + ' · 文件节点 ' + String(st.files.length) + (st.scanNote ? ' · ' + st.scanNote : '')),
        h('button', { className: 'acx-btn', onClick: function () {
          for (const k in manual) delete manual[k]
          store.set({})
        } }, '重排'),
        h('button', { className: 'acx-btn', onClick: function () { setPan({ x: 36, y: 30 }) } }, '复位视图'),
        h('button', { className: 'acx-btn', onClick: function () { store.set({ dock: !store.get().dock }) } }, st.dock ? '收起会话' : '展开会话'),
      )
      ...
      if (form && form.kind === 'agent') {
        ...
        formEl = h('div', { className: 'acx-form' },
          ...
          h('div', { className: 'acx-btns' },
            ...
            h('button', { className: 'acx-btn acx-primary', disabled: formBusy || !formMission.trim(), onClick: submitAgentForm }, formBusy ? '创建中...' : '创建 Agent'),
          ),
        )
      ...
      const canvas = h('div', {
        ...
        h('div', { className: 'acx-content', style: { transform: 'translate(' + String(pan.x) + 'px,' + String(pan.y) + 'px)' } },
          svg,
      ...
      return h('div', { className: 'acx-root' },
        h('div', { className: 'acx-col' }, bar, formEl, canvas),
        st.dock ? h(ChatDock, null) : null,
    ...
    console.log('[agent-canvas] client apply pkg-5: panel "' + PANEL_ID + '" registered')
  },
}
