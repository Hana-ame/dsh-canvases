...
async function main(){
  ...
  // open the graphc owning session (title contains 我希望新建一个dsh插件)
  var click=await ev("(function(){var els=Array.prototype.slice.call(document.querySelectorAll('button,a,div,span,li'));var cands=els.filter(function(e){return /我希望新建一个dsh插件/.test((e.textContent||'')) && e.children.length<=2});if(!cands.length) return 'not-found';var el=cands[cands.length-1];el.click();return 'clicked '+cands.length})()")
  await sleep(7000)
  var probe=await ev("JSON.stringify({root:document.querySelectorAll('.acx-root').length,graph:document.querySelectorAll('.acx-graph').length,perf:document.querySelectorAll('.acx-perf').length,title:document.title,body:document.body.innerText.slice(0,300)})")
