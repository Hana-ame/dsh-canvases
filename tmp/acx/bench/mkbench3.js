...
async function boot(fileCount){
  ...
  for (let i=0;i<fileCount;i++){
    ...
    var ext = (i%2===0) ? '.md' : '.ts'
    entries.push({ path:'/tmp/ws/a1/f'+i+ext, name:'f'+i+ext, rel:'f'+i+ext, type:'file', size: 100+i, depth: 1, __x: originX+col*colW, __y: originY+row*rowH })
  }
  const calls = {}
  const host = { call: function(method, args){
    ...
    if (method==='canvas-read-text') return Promise.resolve({ ok:true, text: '# Title\n\nSome notes here\n- a\n- b\n## Next', truncated:false })
...
fs.writeFileSync('/tmp/acx/bench/bench3.html', `<!doctype html>
...
console.log('bench3.html written', fs.statSync('/tmp/acx/bench/bench3.html').size)
