...
async function main(){
  ...
  var probe=await ev("JSON.stringify({url:location.href, root:document.querySelectorAll('.acx-root').length, graph:document.querySelectorAll('.acx-graph').length, perf:document.querySelectorAll('.acx-perf').length, legends:document.querySelectorAll('.acx-legend').length, btnTexts:Array.prototype.slice.call(document.querySelectorAll('.acx-btn')).map(function(b){return b.textContent}).slice(0,20), bodyLen:document.body.innerHTML.length})")
  console.log(probe)
  fs.writeFileSync('/tmp/acx/bench/gui-probe.json', String(probe))
