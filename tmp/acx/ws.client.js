...
return {
  apply(ctx) {
    ...
    async function refreshInner() {
      ...
      // One canvas = one workspace folder. Sessions own subfolders inside it, so
      // scanning the single root still reveals every session s files.
      ...
      for (let i = 0; i < roots.length; i++) {
        const root = roots[i]
        const r = await hostCall('canvas-list-files', { path: root, maxDepth: 3, limit: 200 }, 15000)
        if (!r.ok) {
    ...
    // One workspace holds every session s files, so ownership is decided per file:
    // the deepest agent cwd (workspace/<session-folder>) that contains it.
        ...
        try {
          ...
          r = timer !== undefined && typeof timer.timeout === 'function'
            ? await Promise.race([work, timer.timeout(20000).then(function () {
                return { ok: false, error: '\u6574\u4f53\u8d85\u65f6 20s\uff1a\u521b\u5efa\u6d41\u7a0b\u672a\u8fd4\u56de', timeout: true }
              })])
            : await work
        } catch (e) {
        ...
        formEl = h('div', { className: 'acx-form' },
          ...
          h('div', { className: 'acx-row' },
            ...
            h('input', {
              className: 'acx-inp',
              value: formCwd,
              placeholder: (store.get().workspace ? store.get().workspace + '/<\u5b50\u76ee\u5f55>...' : '(\u8bf7\u5148\u9009\u5de5\u4f5c\u533a)'),
          ...
          h('div', { className: 'acx-hint' }, '\u7559\u7a7a\u5de5\u4f5c\u76ee\u5f55\u65f6\uff0c\u4f1a\u5728\u5f53\u524d\u5de5\u4f5c\u533a\u5185\u4e3a\u8be5\u4f1a\u8bdd\u65b0\u5efa\u4e00\u4e2a\u5b50\u76ee\u5f55\uff0c\u8fd9\u6837\u6587\u4ef6\u5f52\u5c5e\u6e05\u6670\u3002'),
    ...
    console.log('[agent-canvas] client apply pkg-13: panel "' + PANEL_ID + '" registered, touch=' + String(detectTouch()))
  },
}
