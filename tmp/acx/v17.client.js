...
return {
  apply(ctx) {
    ...
    // A node cached mid-creation has no surviving in-flight request, so restore it
    // as unbound rather than leaving a spinner that can never resolve.
    ...
    async function refreshInner() {
      ...
      for (let i = 0; i < roots.length; i++) {
        const root = roots[i]
        const r = await hostCall('canvas-list-files', { path: root, maxDepth: 8, limit: 4000 }, 20000)
        if (!r.ok) {
        ...
        if (r.truncated) note = root + ' \u5185\u5bb9\u8fc7\u591a\uff0c\u4ec5\u663e\u793a\u524d 4000 \u9879'
      }
    ...
    function CanvasApp() {
      ...
      async function exportMarkdownToWorkspace() {
        ...
        // A workspace that cannot be written here still deserves a deliverable, so
        // fall back to a normal download and report what happened.
      ...
      async function pickWorkspace() {
        ...
        if (ui === undefined || typeof ui.pickDirectory !== 'function') {
          store.set({ wsError: '\u76ee\u5f55\u9009\u62e9\u5668\u4e0d\u53ef\u7528' })
          return
        }
        try {
          const picked = await ui.pickDirectory()
          if (typeof picked !== 'string' || picked.length === 0) return
          store.set({ workspace: picked, wsError: null })
          refresh()
        } catch (e) {
          store.set({ wsError: '\u9009\u62e9\u5de5\u4f5c\u533a\u5931\u8d25\uff1a' + String((e && e.message) || e) })
        }
      }
      ...
      const bar = h('div', { className: 'acx-bar' },
        ...
        }, st.workspace ? '\u5de5\u4f5c\u533a: ' + basename(st.workspace) : '\u9009\u5de5\u4f5c\u533a'),
        h('button', {
      ...
      const canvas = h('div', {
        ...
        st.agents.length === 0
          ? h('div', { className: 'acx-empty', style: { position: 'absolute', left: 0, right: 0, top: '34%' } },
              '\u5148\u70b9\u300c\u9009\u5de5\u4f5c\u533a\u300d\u7ed1\u5b9a\u4e00\u4e2a\u6587\u4ef6\u5939\uff0c\u518d\u7528\u300c+ Agent\u300d\u521b\u5efa\u4f1a\u8bdd\u3002\u6bcf\u4e2a agent \u5728\u8be5\u5de5\u4f5c\u533a\u5185\u62e5\u6709\u81ea\u5df1\u7684\u5b50\u76ee\u5f55\uff0c\u5b83\u521b\u5efa\u7684\u6bcf\u4e2a\u6587\u4ef6\u90fd\u4f1a\u4f5c\u4e3a\u8282\u70b9\u51fa\u73b0\u3002')
          : null,
      )
      let infoSheet = null
    ...
    console.log('[agent-canvas] client apply pkg-17: panel "' + PANEL_ID + '" registered, ws=' + String(store.get().workspace) + ' touch=' + String(detectTouch()))
  },
}
