  ...
  apply(ctx) {
    ...
    function CanvasApp() {
      ...
      function drawGraph() {
        ...
        // Hover highlight: the hovered agent's WHOLE scope lights up, so its subtree
        // reads as one unit. Hovering any file/folder highlights its owning agent too.
        let highlightAgent = hoverAgentId
        if (highlightAgent === null && hoverFile !== null) {
          const hf = fileByPath[hoverFile]
          if (hf !== undefined) highlightAgent = hf.agentId
        }
        let drawn = 0
        ...
        for (let i = 0; i < sections.length; i++) {
          ...
          const eHl = highlightAgent !== null && s.agentId === highlightAgent
        ...
        for (let i = 0; i < sections.length; i++) {
          ...
          g.fill()
          const sHl = highlightAgent !== null && s.agentId === highlightAgent
          g.lineWidth = sHl ? 2 : 1
        ...
        for (let i = 0; i < st.files.length; i++) {
          ...
          g.stroke()
          if (st.fileSel.indexOf(f.path) >= 0 || (highlightAgent !== null && f.agentId === highlightAgent)) {
