  ...
  apply(ctx) {
    ...
    function CanvasApp() {
      ...
      const bar = h('div', { className: 'acx-bar' },
        ...
        CANVAS_ONLY ? null : h('button', { className: 'acx-btn acx-primary', onClick: spawnAgent }, '+ Agent'),
        CANVAS_ONLY ? null : h('button', {
          className: 'acx-btn',
        ...
        CANVAS_ONLY ? null : h('button', { className: 'acx-btn', onClick: openProjectForm }, '投影会话'),
        CANVAS_ONLY ? null : h('button', { className: 'acx-btn', onClick: exportPng }, '导出 PNG'),
        CANVAS_ONLY ? null : h('button', { className: 'acx-btn', onClick: exportJson }, '导出 JSON'),
        CANVAS_ONLY ? null : h('button', { className: 'acx-btn', onClick: exportMarkdown }, '导出 MD'),
        CANVAS_ONLY ? null : h('button', { className: 'acx-btn', onClick: exportMarkdownToWorkspace }, '写入工作区'),
        h('span', { className: 'acx-spacer' }),
        ...
        h('button', { className: 'acx-btn acx-danger', title: '清空缓存、所有节点与分裂状态，视口归位', onClick: resetCanvas }, '重置'),
      )
      let formEl = null
      ...
      const layers = h('div', { className: 'acx-layer' },
        CANVAS_ONLY
          ? (st.workspace ? null : h('div', { className: 'acx-empty', style: { position: 'absolute', left: 0, right: 0, top: '34%' } },
              '点「绑定工作区」选一个文件夹。整个工作区起初只是 1 个节点；点它，在右侧逐个分裂出子文件夹或文件。'))
          : (st.agents.length === 0
      ...
      return h('div', { ref: rootRef, className: touchMode ? 'acx-root acx-touch' : 'acx-root' },
        ...
        legend,
        perfPanel,
        (dirPanel !== null || settingsPanel !== null || (st.dock && !CANVAS_ONLY)) ? h('div', { className: 'acx-right' }, dirPanel, settingsPanel, (st.dock && !CANVAS_ONLY) ? h(ChatDock, { touch: touchMode }) : null) : null,
        formEl,
        infoSheet,
