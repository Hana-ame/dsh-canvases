...
// CANVAS_ONLY: the plugin is, for now, ONLY the canvas. Agent creation, session
// projection, the chat dock and the export buttons are all hidden; the folder
// tree, split, zoom and hover stay. Flip to false to bring them back.
const CANVAS_ONLY = true
const AGENT_SIZE = 56
const FILE_W = 200
const FILE_H = 44
const ROW_PER_COL = 6
...
// World-space dot pitch of the canvas backdrop. The grid is part of the canvas
// world, so it is NOT a fixed CSS backdrop: syncWorld() re-derives its size and
// offset from the same view the bitmap uses. Keep this the single source of
// truth - the .acx-canvas rule below interpolates it.
const GRID = 22
...
const CSS = [
  ...
  // The dot backdrop is part of the canvas WORLD: syncWorld() drives its
  // background-size/position from the live view, so it pans and zooms in lockstep
  // with the graph instead of sitting still under a moving canvas.
  '.acx-canvas{position:absolute;inset:0;z-index:0;pointer-events:none;background-color:var(--dsw-alias-bg-base,transparent);background-image:radial-gradient(var(--dsw-alias-border-l1,rgba(127,127,127,.35)) 1px,transparent 1px);background-size:' + GRID + 'px ' + GRID + 'px}',
  // The annotation is a WORLD-space sibling of the graph, not a screen overlay.
  // It carries the same translate+scale as the bitmap, so dragging the canvas moves
  // the note with it. Deliberately left at z-index auto: it stays BEHIND the graph
  // canvas (z-index 1), which is what makes it a background annotation rather than
  // a card floating over the nodes. It spans the whole canvas (inset:0) so it is a
  // sane containing block - a zero-size one made the absolutely positioned note
  // shrink-to-fit against 0 width and re-wrap as it moved.
  '.acx-world{position:absolute;inset:0;transform-origin:0 0;pointer-events:none;will-change:transform}',
  // The annotation only receives pointer events while its own tool is active; in the
  // other three tools it must stay transparent so the canvas gets the gesture (pan,
  // or node select). Entering its tool also lifts it above the bitmap, otherwise the
  // z-index:1 canvas would swallow the clicks it now needs.
  '.acx-note-mode .acx-world{z-index:2}',
  '.acx-note-mode .acx-note{pointer-events:auto}',
  ...
  // Right-docked property panel for the three select tools. Its body is EMPTY: the
  // brief says to write nothing in it yet, so it carries no rows and no controls.
  // Explicitly a real surface: the overlay token read as see-through here, so the
  // panel looked like it had no background at all. layer-2 is the solid nested
  // surface. Note --dsw-alias-border-l3 does not exist in this theme (only l1/l2),
  // so the border now uses l2 and keeps a rgba fallback.
  '.acx-props{flex:0 1 auto;min-height:0;max-height:42%;overflow:auto;border:1px solid var(--dsw-alias-border-l2,rgba(127,127,127,.35));border-radius:14px;background:var(--dsw-alias-bg-layer-2,var(--dsw-alias-bg-overlay,#202024));box-shadow:0 12px 32px rgba(0,0,0,.28);padding:8px;display:flex;flex-direction:column;gap:6px}',
  // The filelist is the only part allowed to scroll and take the slack, so the split
  // control and 确定 stay pinned and reachable however many files the block holds.
  '.acx-props .acx-slist{flex:1 1 auto;min-height:60px;max-height:none}',
  // Split preview: ONE box cut into an upper and a lower half by a divider. "up" and
  // "down" are conveyed by where each half SITS, not by writing those words into them.
  // Clicking anywhere in the box swaps the two halves.
  '.acx-split{display:flex;flex-direction:column;flex:none;height:110px;border:1px solid var(--dsw-alias-border-l2,rgba(127,127,127,.35));border-radius:10px;overflow:hidden;background:var(--dsw-alias-bg-layer-1,transparent)}',
  '.acx-split>div{flex:1 1 0;min-height:0;display:flex;align-items:center;justify-content:center;font-size:11px;color:var(--dsw-alias-label-secondary,inherit)}',
  '.acx-split>div:first-child{border-bottom:1px dashed var(--dsw-alias-border-l2,rgba(127,127,127,.45))}',
  ...
  // The annotation is a canvas child in WORLD space (panned/zoomed inline), so it
  // has no fixed position of its own; only its own look is defined here.
  // Plain text: no border, no background, no shadow, no padding. The width is applied
  // inline from note.w (edited in the property panel), so it is decided by state and
  // never by position - otherwise dragging changed the shrink-to-fit width and the
  // text re-wrapped.
  '.acx-note{position:absolute;box-sizing:border-box;pointer-events:none;font-size:12px;line-height:1.5;color:var(--dsw-alias-label-secondary,inherit)}',
...
function merge(a, b) {
  ...
  // `note` is a nested object; a plain overwrite would drop its other fields
  // whenever the UI patches just one of them (drag writes x/y, edit writes text).
  if (a && b && a.note && b.note && typeof a.note === 'object' && typeof b.note === 'object') {
    out.note = {}
    for (const k in a.note) out.note[k] = a.note[k]
    for (const k in b.note) out.note[k] = b.note[k]
  }
  return out
}
...
return {
  apply(ctx) {
    ...
    const store = createStore({
      ...
      scanNote: null,
      lastScan: null,
      // The tool always starts as 'pan'. It is deliberately NOT restored from the
      // cache: caches written before the four tools existed hold 'file'/'agent',
      // which would silently open the canvas in a select tool where a drag on empty
      // space does nothing - i.e. the canvas would look undraggable.
      mode: 'pan',
      inspectId: null,
      inspectDir: null,
      manualVersion: 0,
      // Free-form annotation living in WORLD coordinates: it pans and zooms with
      // the graph, so it is pinned to the canvas rather than to the screen.
      note: {
        text: typeof (cached.note && cached.note.text) === 'string' ? cached.note.text : '',
        x: cached.note && typeof cached.note.x === 'number' ? cached.note.x : 24,
        y: cached.note && typeof cached.note.y === 'number' ? cached.note.y : 24,
        active: false,
        // Line width of the annotation in px. It is a property, not a constant: it is
        // edited from the right-hand panel and applied inline to .acx-note.
        w: cached.note && typeof cached.note.w === 'number' && cached.note.w > 0 ? cached.note.w : 260,
      },
      // Whether the background annotation is the current selection (its own tool).
      selNote: false,
      // File blocks of the workspace. null = the DEFAULT single block that holds every
      // file of the workspace; a split replaces it with two blocks, each holding one
      // half. [{ id, paths: string[] }]
      blocks: Array.isArray(cached.blocks) ? cached.blocks : null,
      // Currently selected file block id (its filelist + split control live in the panel).
      blockSel: null,
    })
    const baselines = {}
    const manual = {}
    ...
    function persist() {
      ...
      const noteOut = s.note && typeof s.note === 'object'
        ? { text: typeof s.note.text === 'string' ? s.note.text : '', x: s.note.x, y: s.note.y, w: s.note.w }
        : null
      saveCache({ version: 1, workspace: s.workspace, dock: s.dock, mode: s.mode, agents: agents, positions: positions, revealed: revealedOut, note: noteOut, blocks: Array.isArray(s.blocks) ? s.blocks : null, savedAt: Date.now() })
    }
    ...
    // Side of a file BLOCK node. It stands for a whole group of files - one square, no
    // matter how many files it holds. Its contents are listed in the right-hand panel.
    const BLOCK_SIDE = 150
    ...
    function CanvasApp(slotProps) {
      // The canvas does NOT own workspace selection - that belongs to the app's left
      // sidebar, and only there. It simply FOLLOWS whichever workspace the current
      // session belongs to. Both are standard props of the `main` slot:
      //   useSessions   -> SessionListState { current: SessionId | undefined }
      //   useWorkspaces -> WorkspaceSnapshot { items: WorkspaceView[] }, where each
      //                    WorkspaceView is { workspaceId, path, title, sessionIds }.
      const useSessions = slotProps && typeof slotProps.useSessions === 'function' ? slotProps.useSessions : null
      const useWorkspaces = slotProps && typeof slotProps.useWorkspaces === 'function' ? slotProps.useWorkspaces : null
      const currentSessionId = useSessions !== null
        ? useSessions(function (s) { return (s && s.current) || null })
        : null
      const appWorkspacePath = useWorkspaces !== null
        ? useWorkspaces(function (s) {
            ...
            for (let i = 0; i < items.length; i++) {
              ...
              for (let j = 0; j < ids.length; j++) {
                if (ids[j] === currentSessionId) return items[i].path
              }
            }
            return null
          })
      ...
      // Split flow for a file block. 分列模式 makes the filelist rows clickable; clicking a
      // row moves that file to the OTHER side (the side that will become the new block).
      // 创建新文件块 then turns that side into a block of its own.
      const sp1 = React.useState(false)
      const splitMode = sp1[0]
      const setSplitMode = sp1[1]
      const sp2 = React.useState({})
      const splitMoved = sp2[0]
      const setSplitMoved = sp2[1]
      // Which folders are expanded in the filelist tree. Only an OPEN folder's children are
      // rendered, so a block holding thousands of files still opens instantly.
      const ep1 = React.useState({})
      const expanded = ep1[0]
      const setExpanded = ep1[1]
      ...
      // World-space DOM that must ride the same transform as the bitmap (the dot
      // backdrop and the annotation anchor). Driven imperatively from drawGraph(),
      // never from React state, so panning stays at zero re-renders.
      ...
      const idx = React.useMemo(function () {
        ...
        // The synthetic workspace owner needs an origin of its own: with no x/y the
        // block landed at world y=10, i.e. UNDER the HUD bar, which is why the default
        // file block could not be seen. y=170 puts it clear of a wrapped toolbar.
        if (st.workspace) agentsById[WS_ID] = { id: WS_ID, name: basename(st.workspace), cwd: st.workspace, x: 40, y: 170 }
        ...
        // The workspace owns FILE BLOCKS rather than per-directory cards. By default it
        // has ONE block holding EVERY file of the workspace; a split replaces it with
        // two blocks, each holding one half. Populating s.files is what makes the
        // existing placement / draw / hit-test scaffolding actually put the files
        // INSIDE the block instead of leaving it a collapsed header.
        if (agentsById[WS_ID] !== undefined) {
          const wsOwner = agentsById[WS_ID]
          const allFiles = []
          for (let i = 0; i < st.files.length; i++) {
            const f = st.files[i]
            if (f.type !== 'directory') allFiles.push(f)
          }
          const blockList = Array.isArray(st.blocks) && st.blocks.length > 0
            ? st.blocks
            : [{ id: 'ws-root', paths: allFiles.map(function (f2) { return f2.path }) }]
          for (let bi = 0; bi < blockList.length; bi++) {
            const b = blockList[bi]
            // The section key doubles as the SORT key (sections are sorted by dirRel), so it
            // has to LEAD with the block's index. Keying on the id made 'blk-<stamp>-l' sort
            // before 'blk-<stamp>-u', which drew 文件块 2 ABOVE 文件块 1 after a split.
            let ord = String(bi)
            while (ord.length < 3) ord = '0' + ord
            const sec = ensureSection(wsOwner, '__blk__' + ord + '_' + String(b.id))
            sec.blockId = b.id
            sec.name = '文件块 ' + String(bi + 1)
            sec.depth = 0
            sec.parentKey = null
            sec.dirPath = null
            sec.files = []
            ...
            for (let pi = 0; pi < paths.length; pi++) {
              const f2 = fileByPath[paths[pi]]
              if (f2 !== undefined && f2.type !== 'directory') sec.files.push(f2)
            }
            sec.total = sec.files.length
            sec.done = 0
          }
        }
        for (let i = 0; i < st.files.length; i++) {
        ...
        for (let i = 0; i < sections.length; i++) {
          ...
          s.y = oy - 30 + y0
          if (s.blockId !== undefined) {
            // A file block is ONE square node standing for the whole group. Its files are
            // deliberately NOT written into filePosIndex: that index is what makes the
            // canvas lay every file out as its own node, so a block holding 4000 files
            // exploded into 4000 nodes and blanketed the canvas. The files live in the
            // right-hand filelist instead; only the block itself is drawn.
            const side = BLOCK_SIDE
            s.w = side
            s.h = side
            // A hand-placed block keeps the spot it was dropped on; only blocks the user
            // never moved take the auto-layout slot.
            const mp = manual[blkKey(s.blockId)]
            if (mp !== undefined) { s.x = mp.x; s.y = mp.y }
            cursor[ck] = y0 + s.h + 26
            continue
          }
      ...
      }, [st.files, st.agents, st.manualVersion, st.workspace, st.blocks])
      ...
      // FRAME ONCE on load. pan/zoom are deliberately not persisted, so a refresh used to
      // drop the view back to the default top-left corner - and once the file block moved
      // to y=170, everything the user had arranged looked like it had vanished. Wait for
      // real content, then fit it exactly once. It never re-frames afterwards, so it cannot
      // fight the user's own panning or their manual node placement.
      const autoFitRef = React.useRef(false)
      const filesFitRef = React.useRef(false)
      React.useEffect(function () {
        if (st.agents.length === 0 && st.files.length === 0 && st.workspace === null) return
        if (!autoFitRef.current) {
          autoFitRef.current = true
          fitToContent()
          return
        }
        // The workspace was bound (or its cached files finished scanning) AFTER that first
        // frame: the file block lands well below the old content, so frame once more so it
        // is actually visible. Only once - after this the view belongs to the user.
        if (st.files.length > 0 && !filesFitRef.current) {
          filesFitRef.current = true
          fitToContent()
        }
      }, [st])
      React.useEffect(function () {
      ...
      // A file BLOCK is draggable like any other node, so it needs a live position too.
      // Its hand-placed spot rides the SAME manual[] map the file nodes use, under a
      // namespaced key - no new store field, and the existing persist() path picks it up.
      function blkKey(id) { return '\u0000blk:' + String(id) }
      function blockPosOf(s) {
        const ov = liveRef.current
        if (ov !== null && ov.kind === 'block' && ov.id === s.blockId) return { x: ov.x, y: ov.y }
        const mp = manual[blkKey(s.blockId)]
        if (mp !== undefined) return mp
        return { x: s.x, y: s.y }
      }
      // Apply the live view to the world-space DOM that cannot ride the canvas
      // transform: the dot backdrop and the annotation anchor. Called from
      // drawGraph(), so every path that moves the view - ref-driven drags and
      // state-driven zoom buttons alike - moves backdrop, graph and note as ONE
      // object. Reading viewRef (not props) is what keeps this at zero re-renders.
      function syncWorld() {
        const v = viewRef.current
        const bgEl = bgRef.current
        if (bgEl !== null) {
          const pitch = GRID * v.z
          bgEl.style.backgroundSize = pitch + 'px ' + pitch + 'px'
          bgEl.style.backgroundPosition = v.x + 'px ' + v.y + 'px'
        }
        const worldEl = worldRef.current
        if (worldEl !== null) {
          worldEl.style.transform = 'translate(' + v.x + 'px,' + v.y + 'px) scale(' + v.z + ')'
        }
      }
      function drawGraph() {
        ...
        for (let i = 0; i < sections.length; i++) {
          const s = sections[i]
          // A file BLOCK is a standalone node, not a child of the agent tree: drawing the
          // parent link for it produced a stray dashed curve across the canvas.
          if (s.blockId !== undefined) continue
          const x2 = s.x
          const y2 = s.y + 14
          let x1 = 0
          let y1 = 0
        ...
        for (let i = 0; i < sections.length; i++) {
          const s = sections[i]
          // A block follows its live/hand-placed position; a folder card keeps the layout
          // position the memo assigned it.
          const bx = s.blockId !== undefined ? blockPosOf(s) : s
          if (!vis(bx.x, bx.y, s.w, s.h)) continue
          const isBlock = s.blockId !== undefined
          roundRectPath(g, bx.x, bx.y, s.w, s.h, 12)
          g.fillStyle = pal.dirBg
          g.fill()
          const sHl = inScope(s.dirPath) || (isBlock && st.blockSel === s.blockId)
          g.lineWidth = sHl ? 2 : 1
          g.strokeStyle = (sHl || st.fileSel.indexOf(s.dirPath) >= 0) ? pal.accent : pal.border
          g.stroke()
          if (isBlock) {
            // A block is one square standing for a whole file group: name it, count it,
            // and centre the label so it reads as a single node rather than a folder card.
            g.font = '12.5px ' + FONT_STACK
            g.fillStyle = pal.label
            g.textAlign = 'center'
            g.fillText(s.name, bx.x + s.w / 2, bx.y + s.h / 2 - 8)
            g.font = '10.5px ' + FONT_STACK
            g.fillStyle = pal.dim
            g.fillText(String(s.total) + ' 个文件', bx.x + s.w / 2, bx.y + s.h / 2 + 12)
            g.textAlign = 'left'
            continue
          }
          g.font = '11.5px ' + FONT_STACK
          g.fillStyle = pal.label
          g.fillText(clipText(g, '[D] ' + s.name, s.w - 66), bx.x + 10, bx.y + 15)
          g.font = '10px ' + FONT_STACK
          g.fillStyle = pal.dim
          g.textAlign = 'right'
          g.fillText(String(s.total - s.done) + ' / ' + String(s.total) + ' 项', bx.x + s.w - 10, bx.y + 15)
          g.textAlign = 'left'
        }
        // file nodes
      ...
      function hitTest(wx, wy) {
        ...
        for (let i = sections.length - 1; i >= 0; i--) {
          const s = sections[i]
          const sp = s.blockId !== undefined ? blockPosOf(s) : { x: s.x, y: s.y }
          if (wx >= sp.x && wx <= sp.x + s.w && wy >= sp.y && wy <= sp.y + s.h) {
            if (s.blockId !== undefined) return { kind: 'block', block: s }
      ...
      function onGraphPointerDown(e) {
        ...
        // TOOL 1 - pan. Only this tool moves the view, and it never selects or
        // drags a node: a gesture here is always a pan, whatever is under it.
        // Any unexpected value also lands here, so the canvas can never go dead -
        // scheduling must never depend on `mode` holding one of exactly four strings.
        if (mode !== 'note' && mode !== 'file' && mode !== 'agent') {
          interactionRef.current = { kind: 'pan', sx: e.clientX, sy: e.clientY, px: pan.x, py: pan.y, x: pan.x, y: pan.y }
          return
        }
        // TOOL 2 - annotation. The note only accepts pointer events in this mode
        // (see .acx-note-mode), so a pointerdown that reaches the canvas means the
        // gesture began outside it: drop the selection and deliberately DO NOT pan.
        if (mode === 'note') {
          store.set({ selNote: false })
          return
        }
        const hit = hitTest(p.x, p.y)
        // TOOLS 3/4 - file and agent nodes. Neither pans. A gesture starting on empty
        // space, or on the other kind of node, clears the selection and stops, so the
        // tool only ever acts on its own kind.
        if (mode === 'file') {
          if (hit === null || (hit.kind !== 'file' && hit.kind !== 'block')) {
            store.set({ fileSel: [], inspectDir: null, blockSel: null })
            return
          }
          // A file BLOCK is a node like any other under this tool: it is selected AND it
          // can be dragged, exactly like the agent node in the agent tool.
          if (hit.kind === 'block') {
            const bpos = blockPosOf(hit.block)
            store.set({ blockSel: hit.block.blockId, inspectDir: null, fileSel: [] })
            interactionRef.current = {
              kind: 'block', id: hit.block.blockId, dx: p.x - bpos.x, dy: p.y - bpos.y,
              x0: p.x, y0: p.y, moved: false, rect: rect, x: bpos.x, y: bpos.y,
            }
            return
          }
          const f = hit.file
          ...
          interactionRef.current = {
            ...
            moved: false, canDrag: true, rect: rect, x: pos.x, y: pos.y,
          }
        ...
        if (mode === 'agent') {
          if (hit === null || hit.kind !== 'agent') {
            store.set({ agentSel: [], activeAgentId: null, inspectId: null })
            return
          }
          const agent = hit.agent
          pressedAgentRef.current = agent.id
          interactionRef.current = { kind: 'agent', id: agent.id, canDrag: true, dx: p.x - agent.x, dy: p.y - agent.y, x0: p.x, y0: p.y, moved: false, rect: rect, x: agent.x, y: agent.y }
      ...
      function onGraphPointerMove(e) {
        ...
        if (it.kind === 'block') {
          ...
          it.x = p.x - it.dx
          it.y = p.y - it.dy
          liveRef.current = { kind: 'block', id: it.id, x: it.x, y: it.y }
          scheduleDraw()
        }
      }
      function flushHover() {
      ...
      function onGraphPointerUp(e) {
        ...
        if (it.kind === 'block') {
          // Dropping a block leaves it where it was dropped, so the split nodes do not
          // spring back onto the auto-layout grid.
          if (it.moved) {
            manual[blkKey(it.id)] = { x: Math.round(it.x), y: Math.round(it.y) }
            bumpManual()
          }
        }
      }
      ...
      // ---- CANVAS ANNOTATION ------------------------------------------------
      // The note lives in world coordinates, so it is dragged with the same
      // screen->world inverse the graph uses. Document-level listeners keep the
      // drag alive while the cursor outruns the element.
      function beginNoteDrag(e) {
        if (e.pointerType === 'mouse' && e.button !== 0) return
        if (store.get().note.active) return
        e.stopPropagation()
        const el = canvasRef.current
        ...
        const p = { x: viewRef.current.x, y: viewRef.current.y }
        const n = store.get().note
        const it = { dx: (e.clientX - rect.left - p.x) / z - n.x, dy: (e.clientY - rect.top - p.y) / z - n.y }
        try { e.currentTarget.setPointerCapture(e.pointerId) } catch (err) {}
        const move = function (ev) {
          const nx = (ev.clientX - rect.left - p.x) / z - it.dx
          const ny = (ev.clientY - rect.top - p.y) / z - it.dy
          store.set({ note: { x: Math.round(nx), y: Math.round(ny) } })
        }
        const up = function () {
      ...
      // Create the new block: the selected block keeps the files still on its side, and the
      // files moved to the other side become a block of their own, appended right after it.
      function createBlockFromMoved(bs, keepPaths, movedPaths) {
        const cur = Array.isArray(store.get().blocks) && store.get().blocks.length > 0
          ? store.get().blocks
          : [{ id: 'ws-root', paths: bs.files.map(function (f) { return f.path }) }]
        const stamp = String(Date.now())
        const next = []
        for (let i = 0; i < cur.length; i++) {
          if (cur[i].id !== bs.blockId) { next.push(cur[i]); continue }
          next.push({ id: cur[i].id, paths: keepPaths })
          next.push({ id: 'blk-' + stamp, paths: movedPaths })
        }
        setSplitMode(false)
        setSplitMoved({})
      ...
      // The canvas FOLLOWS the app's active workspace instead of offering a picker of its
      // own. The workspace is chosen in the left sidebar, and a WorkspaceView lists the
      // sessions it owns, so the workspace of the CURRENT session is the one to use.
      // No UI, no dialog: it simply re-points itself when the sidebar selection changes.
      const followedWsRef = React.useRef(null)
      React.useEffect(function () {
        if (appWorkspacePath === null) return
        if (followedWsRef.current === appWorkspacePath) return
        followedWsRef.current = appWorkspacePath
        if (store.get().workspace === appWorkspacePath) return
        store.set({ workspace: appWorkspacePath, wsError: null })
        lastScanRoot = null
        lastRefreshSig = null
        refresh()
      }, [appWorkspacePath])
      ...
      // A real reset: nothing survives it - not the persisted cache, not the split
      // state (manual[] / revealed{}), not the viewport.
      function resetCanvas() {
        ...
        setPan({ x: 36, y: 30 })
        setZoom(1)
        store.set({ agents: [], agentSel: [], activeAgentId: null, workspace: null, fileSel: [], files: [], wsError: null, inspectDir: null, dock: false, manualVersion: (store.get().manualVersion || 0) + 1 })
        refresh()
      }
      ...
      const pickedFiles = st.files.filter(function (f) { return st.fileSel.indexOf(f.path) >= 0 })
      // HUD: the four pointer tools plus zoom, and nothing else. The tools are plain
      // Chinese text buttons - deliberately NOT icon-only.
      const TOOLS = [
        { kind: 'pan', label: '拖动画布' },
        { kind: 'note', label: '选择背景注释' },
        { kind: 'file', label: '选择文件节点' },
        { kind: 'agent', label: '选择 agent 节点' },
      ]
      const bar = h('div', { className: 'acx-bar' },
        TOOLS.map(function (t) {
          const on = st.mode === t.kind
          return h('button', {
            key: t.kind,
            className: on ? 'acx-btn acx-primary' : 'acx-btn',
            onClick: function () { store.set({ mode: t.kind, selNote: false }) },
          }, t.label)
        }),
      ...
      // The annotation is a WORLD-space node, not a screen overlay: its anchor
      // carries the same translate+scale as the bitmap (via syncWorld), so a drag
      // anywhere on the canvas carries the note along with the graph. Coordinates
      // stay in world units, so beginNoteDrag's math is unchanged.
      const noteEl = st.note ? h('div', {
        className: 'acx-world',
        ref: function (el) {
          worldRef.current = el
          if (el !== null) syncWorld()
        },
      }, h('div', {
        className: 'acx-note',
        style: { left: st.note.x + 'px', top: st.note.y + 'px', width: st.note.w + 'px' },
        onPointerDown: function (e) { beginNoteDrag(e) },
        onClick: function (e) { e.stopPropagation(); store.set({ selNote: true }) },
        onDoubleClick: function (e) { e.stopPropagation(); openNoteEditor() },
      }, st.note.active
        ? h('textarea', {
            className: 'acx-ta',
            ...
            onChange: function (e) { store.set({ note: { text: e.target.value } }) },
            onBlur: function () { store.set({ note: { active: false } }) },
            onKeyDown: function (e) { if (e.key === 'Escape') { e.preventDefault(); store.set({ note: { active: false } }) } },
          })
        : h('div', null, st.note.text || '双击这里编辑注释'),
      )) : null
      ...
      const legend = h('div', { className: 'acx-legend' },
        // Plain read-only text. The canvas has NO workspace picker of its own: the app's
        // left sidebar is the only place a workspace is chosen, and this panel follows it.
      ...
      // Right-hand property panel for the select tools. It is rendered ONLY when it has
      // something to show. The file and agent tools have no property rows defined yet,
      // so rendering the panel for them produced an empty framed box that read as a
      // stray "phantom" rectangle in the corner. No content -> no box.
      let propsPanel = null
      if (st.mode === 'note' && st.selNote === true) {
        propsPanel = h('div', { className: 'acx-props' },
          h('div', { className: 'acx-row' },
            h('span', { className: 'acx-sm' }, '宽度'),
            h('input', {
              type: 'range', min: 80, max: 900, step: 10,
              value: st.note.w,
              onChange: function (e) { store.set({ note: { w: Number(e.target.value) } }) },
            }),
            h('span', { className: 'acx-sm' }, String(st.note.w)),
          ),
          h('div', { className: 'acx-sm' }, '文字'),
          h('textarea', {
            className: 'acx-ta',
            value: st.note.text,
            onChange: function (e) { store.set({ note: { text: e.target.value } }) },
      ...
      } else if (st.mode === 'file' && st.blockSel !== null) {
        let bs = null
        for (let i = 0; i < sections.length; i++) {
          if (sections[i].blockId === st.blockSel) { bs = sections[i]; break }
        }
        if (bs !== null) {
          const bpaths = bs.files.map(function (f) { return f.path })
          const isMoved = function (p) { return splitMoved[p] === true }
          const movedPaths = bpaths.filter(isMoved)
          const keepPaths = bpaths.filter(function (p) { return !isMoved(p) })
          // The filelist is a TREE, not a flat dump: every directory is a row that expands to
          // reveal its contents, the way the app's own directory browser reads. Folders come
          // first at each level and only the children of an OPEN folder are rendered, so a
          // block holding thousands of files still opens instantly - the flat list before
          // this had to be truncated to stay operable at all.
          const rootPath = String(st.workspace || '')
          const rootNode = { dirs: {}, files: [], path: '' }
          for (let i = 0; i < bpaths.length; i++) {
            const p = bpaths[i]
            const rel = rootPath !== '' && p.indexOf(rootPath + '/') === 0 ? p.slice(rootPath.length + 1) : p
            const segs = String(rel).split('/')
            let node = rootNode
            let acc = ''
            for (let si = 0; si < segs.length - 1; si++) {
              acc = acc === '' ? segs[si] : acc + '/' + segs[si]
              if (node.dirs[segs[si]] === undefined) node.dirs[segs[si]] = { dirs: {}, files: [], path: acc }
              node = node.dirs[segs[si]]
            }
            node.files.push({ name: segs[segs.length - 1], path: p })
          }
          const listRows = []
          const indentOf = function (depth) { return String(8 + depth * 12) + 'px' }
          const toggleMoved = function (p) {
            const next = {}
            for (const k in splitMoved) next[k] = splitMoved[k]
            if (next[p] === true) delete next[p]
            else next[p] = true
            setSplitMoved(next)
          }
          const fileRow = function (f, depth) {
            const mv = isMoved(f.path)
            // In 分列模式 a click moves this file to the OTHER side; the row dims to show
            // which side it now sits on, so no direction word is written into it.
            listRows.push(h('div', {
              key: 'f:' + f.path,
              className: 'acx-sm',
              title: splitMode ? '点击移动到另一边' : null,
              style: {
                flex: 'none', opacity: mv ? '.45' : '1',
                cursor: splitMode ? 'pointer' : 'default',
                paddingLeft: indentOf(depth),
                overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
              },
              onClick: splitMode ? function () { toggleMoved(f.path) } : null,
            }, f.name))
          }
          const walkNode = function (node, depth) {
            const names = Object.keys(node.dirs).sort()
            for (let i = 0; i < names.length; i++) {
              const d = node.dirs[names[i]]
              const open = expanded[d.path] === true
              listRows.push(h('div', {
                key: 'd:' + d.path,
                className: 'acx-sm',
                style: {
                  flex: 'none', cursor: 'pointer', paddingLeft: indentOf(depth),
                  overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                },
                onClick: function () {
                  const next = {}
                  for (const k in expanded) next[k] = expanded[k]
                  if (next[d.path] === true) delete next[d.path]
                  else next[d.path] = true
                  setExpanded(next)
                },
              }, (open ? '\u25BE ' : '\u25B8 ') + names[i]))
              if (open) walkNode(d, depth + 1)
            }
            const leaves = node.files.slice().sort(function (a, b) { return a.name < b.name ? -1 : (a.name > b.name ? 1 : 0) })
            for (let i = 0; i < leaves.length; i++) fileRow(leaves[i], depth)
          }
          walkNode(rootNode, 0)
          // The split preview is ONE box cut into an upper and a lower half by a divider:
          // the upper half is what THIS block keeps, the lower half is what the new block
          // will take. Which is which is read off the position, not off a label.
          propsPanel = h('div', { className: 'acx-props' },
            h('div', { className: 'acx-sm' }, (splitMode ? '分列模式 · 点击文件移到另一边' : 'filelist') + ' · ' + String(bpaths.length) + ' 项'),
            h('div', { className: 'acx-slist' }, listRows),
            h('div', { className: 'acx-sm' }, '分裂'),
            h('div', { className: 'acx-split' },
              h('div', null, String(keepPaths.length) + ' 项'),
              h('div', null, String(movedPaths.length) + ' 项'),
            ),
            h('div', { className: 'acx-btns', style: { flex: 'none' } },
              h('button', {
                className: 'acx-btn',
                onClick: function () {
              ...
              h('button', {
                className: 'acx-btn acx-primary',
                disabled: movedPaths.length === 0 || keepPaths.length === 0,
                onClick: function () { createBlockFromMoved(bs, keepPaths, movedPaths) },
              }, '创建新文件块'),
            ),
          )
        }
      }
      const rootCls = 'acx-root' + (touchMode ? ' acx-touch' : '') + (st.mode === 'note' ? ' acx-note-mode' : '')
      return h('div', { ref: rootRef, className: rootCls },
        h('div', {
          className: 'acx-canvas',
          ref: function (el) { bgRef.current = el; if (el !== null) syncWorld() },
        }),
        graph,
        layers,
        noteEl,
        ...
        legend,
        perfPanel,
        (propsPanel !== null || dirPanel !== null || settingsPanel !== null || (st.dock && !CANVAS_ONLY)) ? h('div', { className: 'acx-right' }, propsPanel, dirPanel, settingsPanel, (st.dock && !CANVAS_ONLY) ? h(ChatDock, { touch: touchMode }) : null) : null,
        formEl,
        infoSheet,
    ...
    slots.inject('main', function () {
      return slots.register({ name: 'main', key: PANEL_ID }, function (props) { return h(CanvasApp, props) })
    })
